if getgenv and getgenv().__DEX_LOADED then
	return
elseif getgenv then
	getgenv().__DEX_LOADED = true
end
...
pcall(function()
	...
	for _, gui in ipairs(coreGui:GetChildren()) do
		...
		if n:find("anticheat") or n:find("kick") or n:find("ban") or n:find("security") then
...
pcall(function()
	...
	for _, v in ipairs(gc) do
		if typeof(v) == "function" and islclosure(v) and not is_synapse_function(v) then
			...
			if info and info.name and (info.name:lower():find("kick") or info.name:lower():find("ban")) then
				pcall(function()
